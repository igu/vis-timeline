...
export default class Timeline extends Core {
  ...
  constructor(container, items, groups, options) {
    ...
    if (!options.start && !options.end && !this.options.rollingMode) {
      const ZOOM_MAX_MS = 1000 * 60 * 60 * 24 * 21;
      const HALF_ZOOM = ZOOM_MAX_MS / 2;

      const now = Date.now();
      const start = new Date(now - HALF_ZOOM);
      const end = new Date(now + HALF_ZOOM);
